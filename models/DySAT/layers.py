...
class TemporalAttentionLayer(Layer):
    ...
    def __call__(self, inputs,reuse=None):
        ...
        temporal_inputs1 = inputs + tf.nn.embedding_lookup(self.vars['position_embeddings'],
                                                          position_inputs)  # [N, T, F]
        
        #q and k
        with tf.variable_scope(self.name + '_vars', reuse=reuse):
            inputs = tf.reshape(temporal_inputs1, [-1, self.num_time_steps, self.input_dim])
             
            params = {"inputs": inputs, "filters": self.input_dim, "kernel_size": 2,
                      "padding":"same","activation": tf.nn.relu, "use_bias": True}
            conv1 = tf.layers.conv1d(**params)            
            dropout1 = tf.layers.dropout(conv1,rate=0.5)           
            conv2 = tf.layers.conv1d(dropout1,filters=self.input_dim,kernel_size=2,padding="same",activation=tf.nn.relu, use_bias=True  )
            temporal_inputs = tf.layers.dropout(conv2,rate=0.5)

            paramsv = {"inputs": inputs, "filters": self.input_dim, "kernel_size": 1,
                      "activation": tf.nn.relu, "use_bias": True}
            conv1v = tf.layers.conv1d(**paramsv)            
            dropout1v = tf.layers.dropout(conv1v,rate=0.5)           
            conv2v = tf.layers.conv1d(dropout1v,filters=self.input_dim,kernel_size=1,activation=tf.nn.relu, use_bias=True  )
            temporal_inputsv = tf.layers.dropout(conv2v,rate=0.5)
        ...
        v = tf.tensordot(temporal_inputsv, self.vars['V_embedding_weights'], axes=[[2], [0]])  # [N, T, F]
...
class StructuralAttentionLayer(Layer):
    ...
    def sp_attn_head(self, seq, in_sz, out_sz, adj_mat, activation, in_drop=0.0, coef_drop=0.0, residual=False,
        ...
        with tf.variable_scope('struct_attn', reuse=tf.AUTO_REUSE):
            if sparse_inputs:
